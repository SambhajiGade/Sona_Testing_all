...
itemRouter
  ...
  itemRouter
  .route("/items/:Project/:Jj01/:Column3/:Report")
  .get(function (request, response) {
    console.log("GET /items/:itemId");

    var project = request.params.Project;
    var jj01 = request.params.Jj01
    var column3=request.params.Column3
    var report = request.params.Report

    console.log("project : ",project," jj01 : ",jj01," column3 : ",column3," report : ",report )

    Item.find({ Project: project, JJ01: jj01,Column3:column3,Report:report}, function (error, item) {
      if (error) {
        response.status(500).send(error);
        return;
      }
  ...
  .get(function (request, response) {
    ...
    Item.find({ Project: project}, function (error, item) {
      if (error) {
        response.status(500).send(error);
        return;
      }
...
itemRouter
  .route("/items/:Project/:Jj01/:Column3")
  .get(function (request, response) {
    console.log("GET /items/:itemId");
    ...
    Item.find({ Project: project, JJ01: jj01,Column3:column3 }, function (error, item) {
      if (error) {
        response.status(500).send(error);
        return;
      }
